...
## 背景
在Saas/私有化部署的项目开发过程中，有大量的数据报表需求。随着数据规模的增加，在业务层开发的数据报表就遇到了性能问题。复杂业务存在了大量的多表关联，造成数据库压力很大，数据报表的响应时间变长。

受限于产品要提供技术输出能力，降低成本的方式，不能引入公司的数据仓库产品，所以在应用层做了一个轻量级的数据仓库产品，用于解决问题。尝试每天的计划任务，在系统使用低谷时做数据聚合，提供数据报表的查询能力。
...
3. 可伸缩性。
  ...
  随着业务变化，数据量会大幅增加，ETL项目中要具备灵活调整。包括多个ETL项目的时间管理。
