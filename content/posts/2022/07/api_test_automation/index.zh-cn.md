...
### 总结
自动化测试的价值，即使没做过的人也很好理解，但什么时机选择自动化，选择什么类型的，以及是否使用单元测试，使用UI自动化等，都要回归测试的本质。看投入的成本，带来的是否是正向的收益。从这个视角去选择合适的手段解决问题。究其根本，测试是个用来规避风险，而不是创造价值的，很容易被大家轻视。
构建一套接口自动化测试框架并不复杂，引入之后存在的问题也不能忽视。
